import { getProjectTasks, addTask } from './axios-api.js';
import { getErrorText, showError } from './utils.js';
...
const setTaskSubmitListener = (tasksContainer, projectId) => {
  ...
  submitForm.addEventListener('submit', (evt) => {
    ...
    addTask(input.value, projectId)
      ...
      .catch(err => {
        showError(tasksList, getErrorText(err));
      })
      .finally(() => {
...
export const loadProjectTasks = (projectId) => {
  getProjectTasks(projectId)
    .then(res => {
      renderTasks(res.data, projectId)
    }).catch(err => {
